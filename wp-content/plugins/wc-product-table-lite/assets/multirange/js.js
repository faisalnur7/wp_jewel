(function () {
  ...
  var wcpt__multirange = function (input) {
    ...
    function update() {
      var range = max - min;
      if (range === 0) {
        // When min and max are the same (single value), show the bar across full width
        ghost.style.setProperty("--low", "0%");
        ghost.style.setProperty("--high", "100%");
      } else {
        ghost.style.setProperty(
          "--low",
          100 * ((input.valueLow - min) / range) + 1 + "%"
        );
        ghost.style.setProperty(
          "--high",
          100 * ((input.valueHigh - min) / range) - 1 + "%"
        );
      }
    }
